...
@Component({
    selector: 'app-entity-categories-list',
    templateUrl: './entity-categories-list.component.html',
    styleUrls: ['./entity-categories-list.component.scss']
})
export class EntityCategoriesListComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        if (this.currentEntityId > 0) {
            ...
            this.messageService.add({
                ...
                detail: 'Please select an entity to view Entity Categories.'
            });
        }
    }
    ...
    deleteCategory(): void {
        ...
        const sub = this.notificationsService.deleteEntityNotificationCategory(category.id).subscribe({
            next: (response: any) => {
                if (!response?.success) {
