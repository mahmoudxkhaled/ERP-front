...
export class CategoriesListComponent implements OnInit, OnDestroy {
            ...
            const sub = this.notificationsService.listNotificationCategories(this.lastCategoryId, this.filterCount).subscribe({
                next: (response: any) => {
                    ...
                    const systemCategories = Array.isArray(categoriesData) ? categoriesData.map((item: any) => {
                        ...
                        // Check if Entity_ID exists and has a value (not empty string, null, undefined, or 0)
                        ...
                        const isSystemCategory = !hasEntityId;
    ...
    private applySearchFilter(): void {
        ...
        this.filteredCategories = this.categories.filter((category) => {
            ...
            const typeMatch = (category.isSystemCategory ? 'system' : 'entity').includes(searchTerm) || false;
            ...
            return idMatch || titleMatch || descriptionMatch || typeMatch || sendEmailMatch || canUnsubscribeMatch;
        });
    }
